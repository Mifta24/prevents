        ...
        <div class="card mt-2">
            ...
            <div class="card-body">
                ...
                <p><strong>Status:</strong> <span
                        class="bg-success text-white p-1">{{ $registration->payment->status }}</span></p>

            </div>
        </div>
        @if ($registration->payment_status == 'pending')
            <div class="mt-3">
                <form action="{{ route('admin.registration.approve', $registration->id) }}" method="POST"
                    style="display: inline-block;">
                    @csrf
                    <button type="submit" class="btn btn-success">Approve</button>
                </form>
                <form action="{{ route('admin.registration.reject', $registration->id) }}" method="POST"
                    style="display: inline-block;">
                    @csrf
